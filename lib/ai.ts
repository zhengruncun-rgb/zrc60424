...
function buildMock(userInput: string): GenerateResponse {
  ...
  return {
   ...
   result: {
  lecture_outline: `
1）先用两道错例做开场：一题数位错位、一题进位漏加，请学生先判断错在第几步，再说理由，先抓“看出错点”而不是急着重算。
...
`,
  error_analysis: `
错误表现1：竖式里部分积位置偏移，结果差很多。背后原因：位值概念不牢，把“左移一位”当成记忆口号，没有和十位乘法意义对应。讲评抓手：每次写第二行前先说“这一位表示几个十”，再落笔。
...
`,
  remediation: `
基础薄弱学生：①每天5分钟做“对齐模板题”，只练竖式书写与部分积位置，先保证格式稳定；②给进位标记框，要求每做完一列必须把进位圈出并在下一列核对一次。
